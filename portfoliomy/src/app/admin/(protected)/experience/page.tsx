...
export default function ExperienceManagementPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload: Record<string, unknown>) => {
      if (editingId) {
        await updateItem("experiences", editingId, payload);
      } else {
        await addItem("experiences", payload);
      }
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteItem("experiences", id),
  ...
  const reorderMutation = useMutation({
    mutationFn: (orders: { _id: string; priority: number }[]) =>
      reorderItems("experiences", orders),
  ...
  const handleReorder = (newOrder: Experience[]) => {
    ...
    const orders = newOrder.map((exp, index) => ({
      _id: exp._id!,
      priority: index,
    }));
          ...
          <div className="bg-[#171721] border border-white/10 w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl">
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-gray-400">
                    Date Range (e.g., May 2025 - Present)
                  </label>
                  <input
                    required
                    className="w-full bg-[#030014] border border-white/10 rounded-xl px-4 py-2 focus:outline-none focus:border-purple-500 transition-colors"
