...
export default function EducationManagementPage() {
  ...
  const saveMutation = useMutation({
    mutationFn: async (payload: EducationFormData) => {
      if (editingId) {
        await updateItem("education", editingId, payload);
      } else {
        await addItem("education", payload);
      }
    },
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => deleteItem("education", id),
  ...
  const handleReorder = (newOrder: Education[]) => {
    ...
    const orders = newOrder.map((edu, index) => ({
      _id: edu._id!,
      priority: index,
    }));
